...
const {
    isValidRequestBody,
    isValidObjectId, 
    TotalPrice } = require('../utils/validators')
const ProductModel = require('../model/productModel')
...
const updateCart = async (req, res) => {
    try {
        ...
        //cart has 5 items:
        //item.length=5;
        //item[0] = {productId: 1, quantity: 2}
        // so if item[0].productId== cart.item.productid and this productid which here has the same id which is given
        // so here we are removing that item from the cart by using splice
